...
class Phase2(smach.StateMachine):
    def __init__(self, base_controller, head_controller, voice_controller, yolo, tf, pm, context):
        ...
        with self:
            smach.StateMachine.add('START_PHASE_2', Start(voice_controller, context), transitions={'done' : 'GO_TO_TABLE'})
            smach.StateMachine.add('GO_TO_TABLE', GoToTable(base_controller, voice_controller, context), transitions={'done' : 'TAKE_ORDER', 'skip' : 'done'})
